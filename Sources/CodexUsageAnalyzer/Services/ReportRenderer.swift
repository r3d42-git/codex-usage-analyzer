...
enum ReportRenderer {
    static func render(_ result: AnalysisResult) -> String {
        ...
        let bars = sortedDaily.map { row in
            ...
            return "<div class='bar-item' title='\(escape("\(row.date): \(String(format: "%.2f", value)) Credits; \(UsageFormatting.integer(row.totals.totalTokens)) Tokens"))'><div class='bar-value'>\(String(format: "%.0f", value))</div><div class='bar-track'><div class='bar' style='height:\(String(format: "%.2f", height))%'></div></div><div class='bar-label'>\(label)</div></div>"
        }.joined()
        ...
        <header class="header"><div><div class="header-title"><h1>Codex-Verbrauch</h1></div><p class="sub">Lokale Sitzungslogs · erzeugt am \(UsageFormatting.localDate(result.generatedAt))</p></div><div class="total">geschätzte Credits (bekannte Raten)<strong>\(formattedCredits(knownCredits))</strong></div></header>\(warning)
